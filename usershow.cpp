...
UserShow::UserShow(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::UserShow),m_defaulttImg(":/bq/000.png")
{
    ui->setupUi(this);
...
void UserShow::paintEvent(QPaintEvent *event)
{
    ...
    painter.drawRect(-1,0,ui->lb_name->width()+1,this->height());
    ...
    int y=(this->height()-pix.height()-ui->lb_name->height())/2+ui->lb_name->height();
...
{
    //定时检测，接收界面是否超时
    if(m_lastTime.secsTo(QTime::currentTime())>1)
    {
        //设为默认  画面
       slot_setImage( m_defaulttImg);
        //m_lastTime=QTime::currentTime();
    }
}
